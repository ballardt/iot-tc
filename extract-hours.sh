...
if [ "$#" -ne 2 ]
then
  echo "Usage: ./extract-hours.sh <path/to/split-hours/dir> <path/to/output/CSV/dir>"
  exit 1
fi
...
splitDir="${1%/}"
outfile="$curDir/${2%/}/split-hours-features.csv"
new="new.csv"
prev="prev.csv"
cd $splitDir

# Output "device", "date", and "hour" columns
for d in ./*/ ; do (trim=${d:2:-1}; device=${trim%_*}; dateHour=${trim##*_}; date=${dateHour%-*}; hour=${dateHour##*-}; echo "$device,$date,$hour") ; done > $outfile
...
# Output CSV header
echo -e "device,date,hour,sleep_time,active_volume,avg_packet_size,mean_rate,peak_to_mean_rate,active_time,num_servers,num_protocols,unique_dns_reqs,dns_interval,ntp_interval\n$(cat $outfile)" > $outfile
